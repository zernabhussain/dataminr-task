...
export function CustomSwitch(props: propTypes) {
  const { title, component, showExpend, isOpen, onSwitchChange } = props;
  return (
    <div className="custom-switch">
      <Typography sx={{ flexGrow: 1 }}>{title}</Typography>
      {component && component()}
      <AntSwitch defaultChecked name={title} onChange={onSwitchChange} />
...
export default function SwitchComponent(props: propTypes) {
  const { title, component, subList, onSwitchChange } = props;
